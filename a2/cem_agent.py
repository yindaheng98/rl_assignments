...
class CEMAgent(base_agent.BaseAgent):
    ...
    def _eval_candidates(self, candidates):
        ...
        for i in range(n):
            ...
            results = self._rollout_test(num_episodes = self._eps_per_candidate)
            rets[i] = results["mean_return"]
            ep_lens[i] = results["mean_ep_len"]
    ...
    def _compute_new_params(self, params, rets):
        ...
        best_params_idx = np.argsort(rets)[-best_params_n:]
