...
class BCAgent(base_agent.BaseAgent):
    ...
    def _decide_action(self, obs, info):
        ...
        # placeholder
        a = self._model.eval_actor(self._obs_norm.normalize(obs))
        a = self._a_norm.unnormalize(a.sample()).detach()
    ...
    def _compute_actor_loss(self, norm_obs, norm_expert_a):
        ...
        '''
        a = self._model.eval_actor(norm_obs)
        loss = -a.log_prob(norm_expert_a).mean()
        return loss
